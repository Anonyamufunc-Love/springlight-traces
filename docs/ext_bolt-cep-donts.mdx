...
2025/9/26 注：现在终于可以解决这个问题了，下面是解决方案：

#### 逃离 UTF-8

你可以编写自定义的 Rollup 插件来控制代码输出，也就是说，可以编写一个插件，读取代码中的所有中文，然后转成 Unicode 文本。

```ts
function unicodeEscapePlugin() {
  return {
    name: 'unicode-escape',
    transform(code: string, id: string) {
      if (!id.includes('node_modules') && (id.endsWith('.js') || id.endsWith('.ts'))) {
        let chars = 0;
        const transformedCode = code.replace(/[^\x00-\x7F]/g, (char) => {
          chars++;
          return '\\u' + ('0000' + char.charCodeAt(0).toString(16)).slice(-4);
        });
        console.log(`Unicode Escaped ${chars} characters in ${id}`);
        return {
...
之后，查看编译结果会发现中文变成了 Unicode 编码，如 `\u4E2D\u6587`。

```ts title="aeft.ts"
if (!comp || !(comp instanceof CompItem)) {
  return { success: false, message: '未找到活动的合成' };
}
```
